...
2026-09-24. Implemented `work/notes/astra-maa-sword-stab-2026-09-23.md` in `assets_dev/man_at_arms/textured_v3_sword_rig/`. The existing MAA carried its sword in the rigid arm part, so its stab read as a small overhead pitch. The existing loader/shader can use the knight's bent-arm attack path when a separate weapon part and the grip/elbow markers are present.
...
Handoff: `work/handoffs/HANDOFF-maa-sword-stab-2026-09-24.md`. Build commands and pivot/part tables are in the new folder README. The interrupted command left source files but had not exported a model; work resumed from that state without overwriting the baseline.
