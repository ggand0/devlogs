# 042 - PR #34: ThumbnailCache fixes and preview benchmarks

PR: https://github.com/ggand0/viewskater-egui/pull/34 (merged 2026-07-16,
merge commit 570eccd)
...
- Merged into main as PR #34 (2026-07-16). Final pre-merge check: clippy
  clean, 6 tests + ignored benchmark passing, a stray blank line from the
  cache.rs file split amended away (simulation-test commit became e4083ca).
- Numbers independently reproduced before merge via
  `scripts/bench_preview_compare.sh` (local untracked tool that runs both
  branches back to back, cherry-picking the bench commit onto detached
  main): main 142.5/171.5 ms and 35.0 s CPU vs fix 96.4/96.8 ms and
  19.9 s CPU — within ~3% of the table above.
- Commits: 1207fc9 (budget fix), 2f3a198 (decode dedup), 21fdd68
  (GUI bench), e4083ca (simulation test).
